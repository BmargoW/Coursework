...
def date_determination(data_user=None):
    ...
    try:
        if data_user:
            app_logger.info(f"дата введена пользователем")
            ...
            app_logger.info(f"дата введена автоматически")
        ...
        app_logger.warning(f"дата введена некорректно")
...
def my_list(date_of_the_month):
    ...
    for index, row in a.iterrows():
        list_1 = {'last_digits': str(row['Номер карты'])[-4:], 'total_spent': float(row['Сумма платежа']), 'cashback':float(row['Кэшбэк'])}
...
def max_sum(date_of_the_month):
    ...
    for index, row in sorted_by_summa.head(5).iterrows():
        list_2 = {'date': str(row['Дата платежа']), 'amount': float(row['Сумма платежа']),
                  'category':str(row['Категория']), 'description':str(row['Описание'])}
