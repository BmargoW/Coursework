...
from src.utils import date_determination, my_list, max_sum, currency_convert, cost_shares, uploading_content
...
def test_my_list():
    ...
    with patch('pandas.read_excel') as mock_read_excel:
        ...
        assert my_list('2021-12-01 09:21:49') == [{'last_digits': '7197', 'total_spent': 169, 'cashback': 3}]


def test_max_sum():

    mock_df = pandas.DataFrame({
        'Дата платежа': ['01.12.2021', '02.12.2021', '03.12.2021', '04.12.2021', '05.12.2021',
        '07.12.2021','06.12.2021'],
        'Категория': ['супермаркет', 'кафе', 'АЗС', 'автомойка', 'салон', 'кино', 'театр'],
        'Описание': ['доход 1', 'доход 2', 'доход 3', 'доход 4', 'доход 5', 'доход 6', 'доход 7'],
        'Сумма платежа': [300, 50, 1000, 10, 1, 20000, 700]
    })

    with patch('pandas.read_excel') as mock_read_excel:
        mock_read_excel.return_value = mock_df
        assert max_sum('2021-12-07 09:21:49') == [{'amount': 20000.0,
  'category': 'кино',
 ...
 {'amount': 50.0,
  ...
  'description': 'доход 2'}] != [{'amount': 300.0,
  'category': 'супермаркет',
  'date': '2021-12-01 00:00:00',
  'description': 'доход 1'}]
...
def test_cost_shares():
    js_data = {'Meta Data': {'1. Information': 'Monthly Adjusted Prices and Volumes',
                             '2. Symbol': 'AAPL',
                             '3. Last Refreshed': '2026-03-31',
                             '4. Time Zone': 'US/Eastern'},
               'Monthly Adjusted Time Series': {'2026-03-31': {'1. open': '262.4100', '2. high': '266.5300',
                                                               '3. low': '245.5100', '4. close': '253.7900',
